...
int get2dChar(char** array, const int row, const int col) {
    return *((*(array + row)) + col);
...
void set2dChar(char** array, const int row, const int col, char value) {
    *((*(array + row)) + col) = value;
}
...
void print2dCharArray(char** array, const int row, const int col, int offset) {
    char format[7];
    sprintf(format, "%%%dd", offset);
    for (int i = 0; i < row; ++i) {
        for (int j = 0; j < col; ++j) {
            printf(format, get2dChar(array, i, j));
        }
        printf("\n");
    }
}
...
void free2dCharArray(char** array, const int row, const int col) {
    for (int i = 0; i < row; ++i) {
        free(array[i]);
    }
    free(array);
